...
} animehash_context_holder;

animehash_context_holder base_contexts;

void init_animehash_contexts()
{
...
static void animehash(void *state, const void *input)
{
...
//    sph_bmw512_init(&ctx.bmw1);
    ...
    sph_bmw512_close (&ctx.bmw1, hashA);	 //0
	
//    sph_blake512_init(&ctx.blake1);
    sph_blake512 (&ctx.blake1, hashA, 64);    //0
    sph_blake512_close(&ctx.blake1, hashB);   //1
